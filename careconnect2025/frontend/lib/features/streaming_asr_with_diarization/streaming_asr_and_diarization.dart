...
class StreamingAsrAndDiarizationScreen extends StatefulWidget {
  ...
  const StreamingAsrAndDiarizationScreen({super.key, this.allowedCategories, this.patientId, this.onUploadSuccess, this.onUploadError});

  @override
  State<StreamingAsrAndDiarizationScreen> createState() => _StreamingAsrAndDiarizationScreenState();
}

class _StreamingAsrAndDiarizationScreenState extends State<StreamingAsrAndDiarizationScreen> {
  ...
  Future<void> registerSpeakers(Directory directory, sherpa_onnx.SpeakerEmbeddingExtractor extractor, sherpa_onnx.SpeakerEmbeddingManager manager) async {
    List<List<Float32List>> speakerVectors = [];
    List<String> speakerFolders = await getSpeakerDirectories('${directory.path}/voice_samples/');
    List<String> names = [];
    for(int i = 0; i < speakerFolders.length; i++) {
      ...
      for(var file in speakerFiles) {
        Float32List embedding = computeEmbedding(extractor: extractor, filename: file);
  ...
  Future<void> segmentWaveFile(String waveFilename) async {
    ...
    final extractor = sherpa_onnx.SpeakerEmbeddingExtractor(config: embeddingConfig);
    ...
    if(segments.isNotEmpty) {
      ...
      for (int i = 0; i < segments.length; ++i) {
        print(
            '${segments[i].start.toStringAsFixed(3)} -- ${segments[i].end
                .toStringAsFixed(3)}  speaker_${segments[i].speaker}');
        String outputFile = waveFilename.replaceFirst('.wav', '/$i.wav');
        await File(outputFile).create(recursive: true);
        await trimAudio(waveFilename, outputFile, segments[i], extractor, manager);
      }
  ...
  // Trim audio file
  Future<void> trimAudio(String inputPath, String outputPath,sherpa_onnx.OfflineSpeakerDiarizationSegment segment, sherpa_onnx.SpeakerEmbeddingExtractor extractor, sherpa_onnx.SpeakerEmbeddingManager manager) async {
    try{
          ...
          await offlineSpeechRecognizer(outputPath, segment.speaker, extractor, manager);
  ...
  Future<void> offlineSpeechRecognizer(String waveFilename, int speaker, sherpa_onnx.SpeakerEmbeddingExtractor extractor, sherpa_onnx.SpeakerEmbeddingManager manager) async {
    final waveData = sherpa_onnx.readWave(waveFilename);
    ...
    final embedding = computeEmbedding(extractor: extractor, filename: waveFilename);
  ...
  Future<void> _uploadSpeechToTextFileToWeb(String fileName, List<int> fileBytes) async {
    ...
    try {
      ...
      Directory? directory = await getExternalStorageDirectory();
      if(directory != null) {
        File localFile = await File('${directory.path}/notetaker_files/$fileName.txt').create(
            recursive: true);
  ...
  Widget build(BuildContext context) {
    return Column(
      ...
      children: [
        ...
        const SizedBox(height: 16),
        _isLoading ? Column(children: [CircularProgressIndicator(), Text("Processing")]) :
        TextField(
          maxLines: 5,
        ...
        const SizedBox(height: 16),
        ElevatedButton(
          onPressed: _textToDisplay.isNotEmpty
              ? _saveRecognizedText
              : null,
