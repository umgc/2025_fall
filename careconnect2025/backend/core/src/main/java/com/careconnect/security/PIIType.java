...
public enum PIIType {
    ...
    FULL_NAME("Full Name", "\\b[A-Z][a-z]+ [A-Z][a-z]+(?:\\s[A-Z][a-z]+)?\\b", "[NAME_REDACTED]"),
    DATE_OF_BIRTH("Date of Birth", "\\b(?:DOB|Born)[:\\s]*\\d{1,2}[/-]\\d{1,2}[/-]\\d{2,4}\\b", "DOB: [REDACTED]"),

    // Address Information
    ADDRESS("Street Address", "\\b\\d+\\s+[A-Za-z0-9\\s,.-]+(?:Street|St|Avenue|Ave|Road|Rd|Boulevard|Blvd|Lane|Ln)\\b", "[ADDRESS_REDACTED]"),
    ...
    public static PIIType[] getStandardTypes() {
        return new PIIType[]{SSN, EMAIL, PHONE, MEDICAL_ID, FULL_NAME, DATE_OF_BIRTH, CREDIT_CARD};
    }
