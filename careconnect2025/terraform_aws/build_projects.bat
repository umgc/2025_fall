...
REM Produces ZIPs for Amplify (frontend) and Lambda (backend)
REM ==================================================
...
REM =============================
set "FRONTEND_ZIP_OUT=%DIST_DIR%\frontend_web.zip"
...
if /I "%BUILD_FRONTEND%"=="true" (
  ...
  if /I "%FLUTTER_MODE%"=="debug" (
    echo [INFO] flutter build web --debug !FLUTTER_FLAGS!
    call flutter build web --debug !FLUTTER_FLAGS!
  ) else (
    echo [INFO] flutter build web --release !FLUTTER_FLAGS!
    call flutter build web --release !FLUTTER_FLAGS!
  )
  ...
  echo [INFO] Zipping web build to: %FRONTEND_ZIP_OUT%
  if exist "%FRONTEND_ZIP_OUT%" del /f /q "%FRONTEND_ZIP_OUT%" >nul 2>&1
  powershell -NoProfile -Command ^
    "Compress-Archive -Path 'build\web\*' -DestinationPath '%FRONTEND_ZIP_OUT%' -Force"

  if errorlevel 1 ( echo [ERROR] Failed to zip frontend build & popd >nul & exit /b 1 )

  echo [ARTIFACT] %FRONTEND_ZIP_OUT%
  popd >nul
)
...
if /I "%BUILD_FRONTEND%"=="true" (
  if exist "%FRONTEND_ZIP_OUT%" ( echo Frontend : %FRONTEND_ZIP_OUT% ) else ( echo Frontend : FAILED )
)
echo ==================================================
echo Done.
exit /b 0
