...
class _WearablesScreenState extends State<WearablesScreen> {
  ...
  // Temporarily disabled due to health plugin removal
  Future<void> _processHealthDataByType(dynamic allHealthData, String source) async {
    // Health plugin temporarily disabled
    print('⚠ Health data processing disabled - plugin temporarily removed');
                ...
                Row(
                  children: [
                    ...
                    Container(
                      ...
                      decoration: BoxDecoration(
                        color: _getSourceColor(data.source).withValues(alpha:0.1),
                        borderRadius: BorderRadius.circular(8),
                      ),
                      child: Text(
                        data.source,
