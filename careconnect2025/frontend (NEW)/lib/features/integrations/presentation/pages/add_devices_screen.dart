...
class _AddDeviceScreenState extends State<AddDeviceScreen> {
  ...
  // Temporarily disabled due to health plugin removal
  Future<void> _fetchAndLogHealthData(
      dynamic health,
      dynamic types,
      ) async {
    // Health plugin temporarily disabled
    print('⚠ Health data fetch disabled - plugin temporarily removed');
  }
  ...
  Future<void> _connectToAppleHealthReal() async {
    try {
      ...
      // Health plugin temporarily disabled
      print('⚠ Apple Health connection disabled - plugin temporarily removed');
      setState(() {
        isConnecting = false;
        errorMessage = 'Apple Health connection temporarily disabled';
      });
      return;
    } catch (e) {
  ...
  Future<void> _connectToGoogleFitReal() async {
    try {
      ...
      // Health plugin temporarily disabled
      print('⚠ Google Fit connection disabled - plugin temporarily removed');
      setState(() {
        isConnecting = false;
        errorMessage = 'Google Fit connection temporarily disabled';
      });
      return;
    } catch (e) {
